...
import CompagnieAerienne.FenCompagnie;
import CompagnieAerienne.InscriptionCompagnie;
import Directeur.FenDirecteur;
import Operateur.FenOperateur;
import Operateur.InscriptionOperateur;
...
public class Authentification extends JFrame {
	...
	public Authentification() {
		...
		btnSeConnecter.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (usernamedirecteurfield.getText().equals("amine")
						&& passworddirecteurfield.getText().equals("ayk")) {
		...
		button.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				try {
					...
					while (resultat.next()) {
						if (resultat.getString("username").equals(usernameoperateurfield.getText().toString())
								&& resultat.getString("password").equals(passwordoperateurField.getText().toString())) {

							cnx1 = ConnexionMySql.ConnectionDB();
							prepared1 = cnx1.prepareStatement(sql1);
							prepared1.execute();

							String sql2 = "select * from operateurusers where username = '"
									+ usernameoperateurfield.getText().toString() + "'";
				...
				try {
					...
					if (resultat4.next()) {
						JOptionPane.showMessageDialog(null,
								"Vous etes sur la liste Noir !! Veuillez contcter la direction de l'aeroport pour plus de details");
						...
						try {
							...
							while (resultat.next()) {
								if (resultat.getString("username").equals(usernamecompagniefield.getText().toString())
										&& resultat.getString("password")
												.equals(passwordcompagnieField.getText().toString())
										&& resultat.getString("nom").equals(nombox.getSelectedItem().toString())) {
									...
									/*
									 * cnx2=ConnexionMySql.ConnectionDB(); String
									 * sql2="select * from compagnieusers where username ='"+resultat.getString(
									 * "username")+"'"; prepared2=cnx2.prepareStatement(sql2);
									 * resultat2=prepared2.executeQuery(); String nom = resultat2.getString("nom");
									 */
