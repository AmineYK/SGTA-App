		...
		btnSinscrire.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if(lblerror.getText().equals("") && lblerror1.getText().equals("")){
					if(!nomfield.getText().equals("")  && !prenomfield.getText().equals("")  && !nbanneeexprfield.getText().equals("") && !usernamefield.getText().equals("")  && !passwordfield.getText().equals(""))
					 {	
				String sql2 = "select * from operateurusers where nom = '"+nomfield.getText().toString()+"' and prenom = '"+prenomfield.getText().toString()+"'";
                try {
					prepared4 = cnx4.prepareStatement(sql2);
					resultat4=prepared4.executeQuery();
					if(!resultat4.next()){
						JOptionPane.showMessageDialog(null,"Votre nom n'est pas sur la liste des Operateurs ! veuillez vous presenter à la direction !!! ");
					}else{
						  matriculeField.setText(resultat4.getString("matricule"));
						 String sql = "update operateurusers set username= ? , password = ? where nom ='"+nomfield.getText().toString()+"' and prenom = '"+prenomfield.getText().toString()+"'";
						 String er=null;
						 try {
							...
							/*String sql4="insert into nboperateur ( num_operateur , compteur ) values ( ? , ? )";
							cnx4=ConnexionMySql.ConnectionDB();
							prepared4=cnx4.prepareStatement(sql4);
							prepared4.setString(1,matriculeField.getText().toString());
							String ze = String.valueOf(0);
							prepared4.setString(2,ze);
							prepared4.execute();*/
					...
					JOptionPane.showMessageDialog(null,"Numero ou username deja existant ! veuillez le changer");
				}
