	...
	public OperateurVisualiserStatistiques() {
		...
		btnVoirNombresDaeronefs_1.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				String sql ="select count(*) from vol where nom = '"+compagniebox.getSelectedItem().toString()+"' and action = '"+id+"'";
				int cpt = 0;
				try {
					prepared=cnx.prepareStatement(sql);
					resultat=prepared.executeQuery();
		...
		btnVoirLeGraphe.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				try {
					...
					JFreeChart chart = ChartFactory.createBarChart("Courbe des Pistes","numero de la piste","Son utilisation", dataset,PlotOrientation.VERTICAL,false,true,true);
