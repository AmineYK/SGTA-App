...
public class FenDirecteur extends JFrame {
	...
	public FenDirecteur() {
		...
		btnVoirLaCompagnie_1.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				/*
				 * String sql = "select * from nboperateur"; int max = 0 ;int a ;String s;
				 * String num = null; try { prepared=cnx.prepareStatement(sql);
				 * resultat=prepared.executeQuery(); while(resultat.next()){ s =
				 * resultat.getString("compteur"); a=Integer.valueOf(s); if(a>max){ max = a ; }
				 * } String Max = String.valueOf(max); String
				 * sql1="select * from nboperateur where compteur = "+Max+"";
				 * cnx1=ConnexionMySql.ConnectionDB(); cnx2=ConnexionMySql.ConnectionDB();
				 * prepared1=cnx1.prepareStatement(sql1); resultat1=prepared1.executeQuery();
				 * if(resultat1.next()){ num =resultat1.getString("num_operateur"); }
				 * 
				 * String sql2="select * from operateurusers where matricule = "+num+"";
				 * prepared2=cnx2.prepareStatement(sql2); resultat2=prepared2.executeQuery();
				 * if(resultat2.next()){ operateurfield.setText(resultat2.getString("nom") +" "+
				 * resultat2.getString("prenom")); }
				 * 
				 * 
				 * 
		...
		btnVoirLaCompagnie.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				/*
				 * String sql ="select * from nbcompagnie"; String aa=null;int a;int max=0; try
				 * { prepared=cnx.prepareStatement(sql); resultat=prepared.executeQuery();
				 * while( resultat.next()){ aa=resultat.getString("compteur");
				 * a=Integer.valueOf(aa); if(a>max){ max=a; } } String cpt=String.valueOf(max);
				 * String sql1="select * from nbcompagnie where compteur ="+cpt+"";
				 * cnx1=ConnexionMySql.ConnectionDB(); prepared1=cnx1.prepareStatement(sql1);
				 * resultat1=prepared1.executeQuery(); if(resultat1.next()){
				 * comfield.setText(resultat1.getString("nom")); } } catch (SQLException e1) {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (ligne == -1) {
					...
					int a = JOptionPane.showConfirmDialog(null,
							"Voulez-vous vraiment la supprimer de la Liste Noire ??", "Suppression de la Liste Noire",
